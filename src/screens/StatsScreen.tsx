...
const StatsScreen: React.FC = () => {
  ...
  return (
    <ScrollView style={tw`flex-1 bg-stone-50 mt-8`} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
      {/* Header Section - Minimal */}
      <View style={tw`px-5 pt-6 pb-4`}>
        <View style={tw`flex-row justify-between items-center`}>
          <View>
            <Text style={tw`text-3xl font-light text-black`}>Statistics</Text>
            <Text style={tw`text-sm text-sand-500 mt-1`}>{format(new Date(), 'EEEE, MMMM d')}</Text>
          </View>
      ...
      <View style={tw`mx-5 mb-6`}>
        <TouchableOpacity onPress={() => setShowAIPrediction(!showAIPrediction)} activeOpacity={0.8} style={tw`bg-sand rounded-3xl overflow-hidden shadow-sm`}>
          <LinearGradient colors={['#FAFAFA', '#FFFFFF']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={tw`p-6`}>
            <View style={tw`flex-row justify-between items-center`}>
              <View style={tw`flex-row items-center gap-3`}>
                ...
                <View>
                  ...
                  <Text style={tw`text-xs text-sand-500 mt-0.5`}>Personalized predictions</Text>
                </View>
              </View>
            ...
            {showAIPrediction && (
              <View style={tw`mt-6 pt-6 border-t border-stone-100`}>
                {loadingPrediction ? (
                  <View style={tw`py-8 items-center`}>
                    <ActivityIndicator size="small" color="#000000" />
                    <Text style={tw`text-xs text-stone-300 mt-3`}>Analyzing patterns...</Text>
                  </View>
                ) : (
                  aiPrediction && (
                    <View>
                      {/* Success Prediction */}
                      <View style={tw`mb-6`}>
                        <Text style={tw`text-xs uppercase tracking-wider text-stone-300 mb-3`}>Success Prediction</Text>
                        <View style={tw`flex-row items-center justify-between`}>
                          <View style={tw`flex-row items-baseline gap-1`}>
                            ...
                            <Text style={tw`text-2xl font-light text-stone-300`}>%</Text>
                          </View>
                      ...
                      {/* Key Insight */}
                      <View style={tw`bg-stone-50 rounded-2xl p-4 mb-4`}>
                        <Text style={tw`text-xs text-sand-500 mb-1`}>Key Insight</Text>
                        <Text style={tw`text-sm text-black`}>{aiPrediction.keyInsight}</Text>
                      </View>

                      {/* Recommendation */}
                      <View style={tw`bg-stone-50 rounded-2xl p-4 mb-4`}>
                        <Text style={tw`text-xs text-sand-500 mb-1`}>Recommendation</Text>
                        <Text style={tw`text-sm text-black`}>{aiPrediction.recommendation}</Text>
                      </View>

                      {/* Predicted Metrics */}
                      <View style={tw`flex-row gap-3`}>
                        <View style={tw`flex-1 bg-stone-50 rounded-2xl p-4`}>
                          <Text style={tw`text-2xl font-light text-black mb-1`}>{aiPrediction.predictedStreak}d</Text>
                          <Text style={tw`text-xs text-sand-500`}>Predicted streak</Text>
                        </View>
                        <View style={tw`flex-1 bg-stone-50 rounded-2xl p-4`}>
                          <Text style={tw`text-2xl font-light text-black mb-1`}>{aiPrediction.confidence}%</Text>
                          <Text style={tw`text-xs text-sand-500`}>Confidence</Text>
                        </View>
                      </View>
                    </View>
                  )
      ...
      <View style={tw`mx-5 mb-6`}>
        <View style={tw`bg-sand rounded-3xl p-5 shadow-sm`}>
          <Text style={tw`text-xs uppercase tracking-wider text-stone-300 mb-4`}>Current Performance</Text>
          <View style={tw`flex-row justify-between`}>
            <View style={tw`flex-1 items-center`}>
              <Text style={tw`text-2xl font-light text-black mb-1`}>{avgCompletion}%</Text>
              <Text style={tw`text-xs text-sand-500`}>Completion</Text>
            </View>

            <View style={tw`w-px bg-sand-100`} />

            <View style={tw`flex-1 items-center`}>
              <Text style={tw`text-2xl font-light text-black mb-1`}>{currentStreak}d</Text>
              <Text style={tw`text-xs text-sand-500`}>Best Streak</Text>
            </View>
            ...
            <View style={tw`flex-1 items-center`}>
              ...
              <Text style={tw`text-xs text-sand-500`}>Total XP</Text>
            </View>
          </View>
        </View>
      </View>
      ...
      <View style={tw`mx-5 mb-6`}>
        <Text style={tw`text-xs uppercase tracking-wider text-stone-300 mb-4`}>Achievements</Text>
        <View style={tw`flex-row flex-wrap gap-3`}>
          <View style={tw`flex-1 min-w-[47%] bg-sand rounded-2xl p-4 shadow-sm`}>
            <View style={tw`flex-row items-center gap-3`}>
              <Award size={16} color="#000000" />
              <View>
                <Text style={tw`text-xl font-light text-black`}>{totalCompletions}</Text>
                <Text style={tw`text-xs text-sand-500`}>Completions</Text>
              </View>
            </View>
          </View>
          ...
          <View style={tw`flex-1 min-w-[47%] bg-sand rounded-2xl p-4 shadow-sm`}>
            <View style={tw`flex-row items-center gap-3`}>
              ...
              <View>
                ...
                <Text style={tw`text-xs text-sand-500`}>Active</Text>
              </View>
            </View>
          </View>
          ...
          <View style={tw`flex-1 min-w-[47%] bg-sand rounded-2xl p-4 shadow-sm`}>
            <View style={tw`flex-row items-center gap-3`}>
              ...
              <View>
                ...
                <Text style={tw`text-xs text-sand-500`}>Perfect Days</Text>
              </View>
            </View>
          </View>
          ...
          <View style={tw`flex-1 min-w-[47%] bg-sand rounded-2xl p-4 shadow-sm`}>
            <View style={tw`flex-row items-center gap-3`}>
              ...
              <View>
                ...
                <Text style={tw`text-xs text-sand-500`}>Current</Text>
              </View>
            </View>
          </View>
        </View>
      ...
      <View style={tw`mx-5 mb-6`}>
        <View style={tw`bg-sand rounded-3xl p-5 shadow-sm`}>
          <View style={tw`flex-row justify-between items-center mb-3`}>
            <Text style={tw`text-xs uppercase tracking-wider text-stone-300`}>Level Progress</Text>
            <Text style={tw`text-xs text-sand-500`}>
