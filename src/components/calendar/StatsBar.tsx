// src/components/calendar/StatsBar.tsx
  ...
  return (
    <View style={tw`bg-sand mx-5 mt-4 rounded-2xl p-4 shadow-sm`}>
      <View style={tw`flex-row justify-around`}>
        <StatItem label={t('calendar.stats.current')} value={habit.currentStreak} />
        <Divider />
        <StatItem label={t('calendar.stats.best')} value={habit.bestStreak} />
        <Divider />
        <StatItem label={t('calendar.stats.total')} value={habit.completedDays.length} />
        <Divider />
        <StatItem label={t('calendar.stats.tier')} value={`${Math.round(progress)}%`} valueColor={theme.accent} />
      </View>
    </View>
  );
};
