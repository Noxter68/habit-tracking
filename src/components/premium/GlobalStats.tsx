...
const GlobalStats: React.FC<GlobalStatsProps> = ({ globalStats, habits, onRefresh }) => {
  return (
    <ScrollView showsVerticalScrollIndicator={false}>
      <View style={tw`bg-sand rounded-3xl px-5 py-6`}>
        {/* Header */}
        <View style={tw`flex-row justify-between items-center mb-8`}>
          <Text style={tw`text-xs uppercase tracking-wider text-stone-300`}>Overview</Text>
          {onRefresh && (
            <TouchableOpacity onPress={onRefresh} style={tw`bg-stone-50 px-3 py-1 rounded-full`}>
              <Text style={tw`text-xs text-sand-500`}>Refresh</Text>
            </TouchableOpacity>
          )}
        </View>
        ...
        <View style={tw`flex-row flex-wrap gap-3 mb-8`}>
          <View style={tw`flex-1 min-w-[47%] bg-stone-50 rounded-2xl p-5`}>
            <Text style={tw`text-3xl font-light text-black`}>{globalStats.totalHabits}</Text>
            <Text style={tw`text-xs text-stone-300 mt-2 uppercase tracking-wider`}>Habits</Text>
          </View>
          <View style={tw`flex-1 min-w-[47%] bg-stone-50 rounded-2xl p-5`}>
            <Text style={tw`text-3xl font-light text-black`}>{globalStats.averageStreak}</Text>
            <Text style={tw`text-xs text-stone-300 mt-2 uppercase tracking-wider`}>Avg Streak</Text>
          </View>
          <View style={tw`flex-1 min-w-[47%] bg-stone-50 rounded-2xl p-5`}>
            <Text style={tw`text-3xl font-light text-black`}>{globalStats.totalCompletions}</Text>
            <Text style={tw`text-xs text-stone-300 mt-2 uppercase tracking-wider`}>Completed</Text>
          </View>
          <View style={tw`flex-1 min-w-[47%] bg-stone-50 rounded-2xl p-5`}>
            <Text style={tw`text-3xl font-light text-black`}>{globalStats.perfectDays}</Text>
            <Text style={tw`text-xs text-stone-300 mt-2 uppercase tracking-wider`}>Perfect Days</Text>
          </View>
        </View>

        {/* Level & XP - Minimal Bar with rounded background */}
        <View style={tw`mb-8 bg-stone-50 rounded-2xl p-4`}>
          <View style={tw`flex-row justify-between mb-2`}>
            <Text style={tw`text-xs text-gray-600`}>Level {globalStats.currentLevel}</Text>
            <Text style={tw`text-xs text-stone-300`}>{globalStats.totalXP} XP</Text>
          </View>
          <View style={tw`h-1 bg-stone-200 rounded-full overflow-hidden`}>
        ...
        <View>
          <Text style={tw`text-xs uppercase tracking-wider text-stone-300 mb-4`}>Habits</Text>
          {habits.map((habit, index) => {
            const progress = habit.completedDays ? Math.round((habit.completedDays.length / Math.max(habit.totalDays, 1)) * 100) : 0;

            return (
              <View key={habit.id} style={tw`mb-4 bg-stone-50 rounded-2xl p-4`}>
                <View style={tw`flex-row justify-between items-center mb-3`}>
                  <Text style={tw`text-sm text-stone-800`}>{habit.name}</Text>
