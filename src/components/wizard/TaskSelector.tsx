...
import { CheckCircle, Circle, Sparkles, Lightbulb } from 'lucide-react-native';
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={tw`px-6 pb-4`}>
        ...
        <View style={tw`gap-3 mb-6`}>
          {availableTasks.map((task, index) => {
            ...
            return (
              <Animated.View key={task.id} entering={FadeInDown.delay(index * 40).duration(400)}>
                <Pressable
                  onPress={() => !isMaxed && toggleTask(task.id)}
                  disabled={isMaxed}
                  style={({ pressed }) => [tw`rounded-2xl overflow-hidden`, pressed && !isMaxed && tw`opacity-90`, isMaxed && tw`opacity-50`]}
                >
                  <LinearGradient
                    colors={isSelected ? [gradientColors[0] + 'E6', gradientColors[1]] : ['#FAFAFA', '#F5F5F5']}
                    style={[tw`p-4 flex-row items-center`, isSelected ? { borderWidth: 2, borderColor: gradientColors[1] + 'DD' } : { borderWidth: 2, borderColor: '#E5E7EB' }]}
                  >
                    {/* Selection Icon */}
                    <View style={tw`mr-3`}>{isSelected ? <CheckCircle size={24} color="#ffffff" strokeWidth={2} /> : <Circle size={24} color="#d1d5db" strokeWidth={2} />}</View>

                    {/* Task Emoji */}
                    <Text style={tw`text-2xl mr-3`}>{task.icon}</Text>

                    {/* Task Info */}
                    <View style={tw`flex-1`}>
                      <Text style={[tw`text-base font-semibold mb-0.5`, isSelected ? tw`text-white` : tw`text-quartz-800`]}>{task.name}</Text>
                      <Text style={[tw`text-sm`, isSelected ? tw`text-white/90` : tw`text-quartz-600`]}>{task.description}</Text>
                      {task.duration && <Text style={[tw`text-xs mt-1`, isSelected ? tw`text-white/80` : tw`text-quartz-500`]}>⏱ {task.duration}</Text>}
                    </View>
                  </LinearGradient>
                </Pressable>
              </Animated.View>
        ...
        <View style={tw`bg-emerald-50 rounded-2xl p-4 border-l-3 border-l-emerald-500`}>
          <View style={tw`flex-row items-center mb-2`}>
            <Lightbulb size={18} color="#059669" strokeWidth={2} style={tw`mr-2`} />
            <Text style={tw`text-sm font-semibold text-emerald-900`}>{tips.tasks[0].title}</Text>
          </View>
          <Text style={tw`text-sm text-emerald-800 leading-5`}>{tips.tasks[0].content}</Text>
        </View>
      </ScrollView>
    </View>
  );
