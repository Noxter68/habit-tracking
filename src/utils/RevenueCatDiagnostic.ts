...
export const diagnoseRevenueCatSetup = () => {
  ...
  Logger.debug('   EXPO_PUBLIC_REVENUECAT_API_KEY exists:', typeof EXPO_PUBLIC_REVENUECAT_API_KEY !== 'undefined');
  // Logger.debug('   REVENUECAT_ANDROID_API_KEY exists:', typeof REVENUECAT_ANDROID_API_KEY !== 'undefined');

  if (typeof EXPO_PUBLIC_REVENUECAT_API_KEY === 'undefined') {
  ...
  if (Platform.OS === 'ios') {
    ...
    if (!EXPO_PUBLIC_REVENUECAT_API_KEY) {
      Logger.error('   ❌ iOS API key is empty or undefined');
      Logger.error('   → Add EXPO_PUBLIC_REVENUECAT_API_KEY=appl_YOUR_KEY to .env file\n');
      return;
    }

    Logger.debug('   Key exists: ✅');
    Logger.debug('   Key length:', EXPO_PUBLIC_REVENUECAT_API_KEY.length);
    Logger.debug('   Key preview:', EXPO_PUBLIC_REVENUECAT_API_KEY.substring(0, 15) + '...');
    Logger.debug('   Starts with appl_:', EXPO_PUBLIC_REVENUECAT_API_KEY.startsWith('appl_'));

    if (!EXPO_PUBLIC_REVENUECAT_API_KEY.startsWith('appl_')) {
      Logger.error('\n   ❌ WRONG KEY FORMAT!');
      ...
      Logger.error('   5. Paste in .env as EXPO_PUBLIC_REVENUECAT_API_KEY=appl_...\n');
      return;
    }

    if (EXPO_PUBLIC_REVENUECAT_API_KEY.length < 30) {
