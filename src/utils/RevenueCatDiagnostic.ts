...
export const diagnoseRevenueCatSetup = () => {
  ...
  Logger.debug('   REVENUECAT_API_KEY exists:', typeof REVENUECAT_API_KEY !== 'undefined');
  // Logger.debug('   REVENUECAT_ANDROID_API_KEY exists:', typeof REVENUECAT_ANDROID_API_KEY !== 'undefined');

  if (typeof REVENUECAT_API_KEY === 'undefined') {
  ...
  if (Platform.OS === 'ios') {
    ...
    if (!REVENUECAT_API_KEY) {
      ...
      Logger.error('   → Add REVENUECAT_API_KEY=appl_YOUR_KEY to .env file\n');
      return;
    }

    Logger.debug('   Key exists: ✅');
    Logger.debug('   Key length:', REVENUECAT_API_KEY.length);
    Logger.debug('   Key preview:', REVENUECAT_API_KEY.substring(0, 15) + '...');
    Logger.debug('   Starts with appl_:', REVENUECAT_API_KEY.startsWith('appl_'));
    ...
    if (!REVENUECAT_API_KEY.startsWith('appl_')) {
      ...
      Logger.error('   5. Paste in .env as REVENUECAT_API_KEY=appl_...\n');
      return;
    }

    if (REVENUECAT_API_KEY.length < 30) {
